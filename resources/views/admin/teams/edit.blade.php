            ...
            <form method="POST" action="{{ route('admin.teams.update', $team->id) }}" enctype="multipart/form-data">
                ...
                <div class="grid gap-6 mb-6 md:grid-cols-1">
                    <div>
                        ...
                        <input type="text" name="name" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" value="{{ $team->name }}" pattern="[A-Za-z0-9\s]+" required>
                    </div>
                    <div>
                        <label for="short_name" class="block mb-2 text-sm font-medium text-gray-900">Short Name</label>
                        <input type="text" name="short_name" id="short_name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" value="{{ $team->short_name }}" pattern="[A-Z0-9]{3}" required>
                    </div>
                    <div>
                        <label for="manager" class="block mb-2 text-sm font-medium text-gray-900">Manager</label>
                        <input type="text" name="manager" id="manager" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" value="{{ $team->manager }}" pattern="[A-Za-z0-9\s]+" required>
                    </div>
                    <div>
